...
var k=0;
//class name
var c1="gridxCellWidget";
var c2="dijitReset dijitInputInner";
var c3="dijitCalendarEnabledDate dijitCalendarCurrentMonth dijitCalendarDateTemplate dijitCalendarSelectedDate";

//alert for counting grid rows
var grid = new Array();
var grid[1-1]=Number(window.prompt("Direct Shareholders"));
var grid[2-1]=Number(window.prompt("List the indirect intermediate and ...."));
var grid[3-1]=Number(window.prompt("Do the following individuals ...."));
...
var IGnum=document.getElementsByClassName(c1).length+1;
	...
	for(count1=0;count1<row_n*col_n;count1++){
		...
		if(dateOrnot(data[count1]) == false){
			...
			if(data[count1]=="true" || data[count1]=="false"){
				document.getElementsByClassName(c1)[index].dispatchEvent(event);
				...
				document.getElementsByClassName(c1)[index].dispatchEvent(event);
				dijit.byId(document.getElementsByClassName(c1)[index].Id).setCellValue(data[count1],data[count1],document.getElementsByClassName(c1)[index].Id,true);
				index++;
			}
		}
		//calendar
			...
			document.getElementsByClassName(c1)[index].dispatchEvent(event);
			document.getElementsByClassName(c2)[53].value=data[count1];
			dijit.byId(document.getElementsByClassName(c2)[53].id).openDropDown();
			document.getElementsByClassName(c3)[document.getElementsByClassName(c3).length-1].click();
			index++;
		}
	return index;
	}
...
function loadexcel1(cif,cpname){
	...
	for(count=0;count<129;count++){
		document.getElementsByClassName(c2)[count].value=data[count];
	}
};
	...
	for(count=0;count<grid.length;count++){
		...
		for(row;row<row+grid[count];row++){
			for(col=3;col<col_n+3;col++){			
				excel_sheet.Cells(row,col).Value=document.getElementsByClassName(c1)[firstindex].innerHTML;
				firstindex++;
			}
		}
	}
...
document.getElementsByClassName(c1)[companynum].dispatchEvent(event);
...
cif=document.getElementsByClassName(c2)[6].value;
companyname=document.getElementsByClassName(c2)[7].value;
